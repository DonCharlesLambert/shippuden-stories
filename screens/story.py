...
class StoryScreen:
    ...
    def speak(self, speaker, text, side=LEFT):
        ...
        for i in range(0, len(text)):
            ...
            self.animate()
            if self.skip_text and i == len(text) - 1:
                break
        self.await_press = True
        while self.await_press:
            self.animate()
    ...
    def appear(self, name, x):
        ...
        if fighter is None:
            ...
            self.present_characters[name] = create_fighter(
                self.canvas, False, name, initial_positon, True
            )
    ...
    def teleport(self, name, x):
        ...
        teleport_item = self.canvas.create_image(
            x, self.floor_height, image=teleport_image, anchor="s"
        )
        for i in range(1, 6):
            teleport_image = ImageTk.PhotoImage(
                Image.open(rf"sprites\common\teleport\{i}.png")
            )
    ...
    def get_mug(self, name=CharacterNames.NARUTO, side=LEFT):
        ...
        canvas_item = self.canvas.create_image(
            0 if side == LEFT else self.WIDTH - img.width,
            0,
            image=mug_image,
            anchor="nw",
        )
    ...
    def set_dialogue(self, side=LEFT):
        ...
        canvas_item = self.canvas.create_image(
            0 if side == LEFT else self.WIDTH - width, 80, image=bg_image, anchor="nw"
        )
        return canvas_item
