...
class SelectScreen:
    ...
    def select_screen(self):
        ...
        self.player_one_icon = self.get_character_select(
            "1p", self.selected_characters[0]
        )
        self.player_two_icon = self.get_character_select(
            "2p", self.selected_characters[1], offset=True
        )
        self.mock_player_one = create_fighter(
            self.canvas,
            False,
            names[self.selected_characters[0]],
            PLAYER_ONE_POSITION,
            character_select=True,
        )
        self.mock_player_two = create_fighter(
            self.canvas,
            False,
            names[self.selected_characters[1]],
            PLAYER_TWO_POSITION,
            character_select=True,
        )

        self.selekta = Selekta(
            self.canvas, PLAYER_ONE_POSITION[0], PLAYER_ONE_POSITION[1]
        )
    ...
    def key_press(self, e):
        if e.char == "s":
            self.selected_characters[self.current_choosing] = (
                self.selected_characters[self.current_choosing] + 1
            ) % len(CharacterNames)
        if e.char == "w":
            self.selected_characters[self.current_choosing] = (
                self.selected_characters[self.current_choosing] - 1
        ...
        if e.char in ["a", "d"]:
            selekta_x, selekta_y = (
                PLAYER_ONE_POSITION
                if self.current_choosing == 0
                else PLAYER_TWO_POSITION
            )
...
class Selekta:
    ...
    def create(self, x, y):
        ...
        return self.canvas.create_image(
            x + self.x_offset, y + self.y_offset, image=self.img, anchor="n"
        )
