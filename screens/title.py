...
class TitleScreen:
    ...
    def title_screen(self):
        ...
        self.demo_character = create_fighter(
            self.canvas,
            False,
        ...
        for i in range(2):
            ...
            self.images.append(
                self.get_title_button(position=i, text=self.button_texts[i])
            )
    ...
    def get_title_button(self, position=0, text=None):
        img = (
            Image.open(rf"sprites\misc\btn.png")
            if text is None
            else Image.open(rf"sprites\misc\{text}.png")
        )
        ...
        if text is None:
            img = img.resize(
                (self.button_width, self.button_height), Image.Resampling.LANCZOS
            )
    ...
    def get_coords(self, position, text=None, text_width=None):
        ...
        y = (
            position * 65
            + 165
            + ((self.button_height - self.text_height) / 2 if text else 0)
        )
        return x, y
...
class Selekta:
    ...
    def create(self, x, y):
        ...
        return self.canvas.create_image(
            x + self.x_offset, y + self.y_offset, image=self.img, anchor="nw"
        )
